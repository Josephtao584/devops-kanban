      ...
      <section class="detail-hero surface-panel">
        <div class="detail-hero__inner">
          ...
          <div class="detail-hero__content">
            <p class="detail-hero__eyebrow">{{ $t('dashboard.detail.project') }}</p>
            <h1 class="detail-hero__title">{{ detail?.project?.name || id }}</h1>
            <p v-if="detail?.team?.name" class="detail-hero__sub">{{ $t('dashboard.scope.team') }} · {{ detail.team.name }}</p>
          </div>
      ...
      <template v-if="detail">
        <section class="metric-grid">
          ...
          <div class="metric-card surface-card">
            ...
            <div class="metric-card__values">
              <div class="metric-card__value">
                <span class="metric-card__number">{{ detail.tasks.recent7dDone }}</span>
                <span class="metric-card__sub">{{ recentLabel }}</span>
              </div>
              <div class="metric-card__divider"></div>
              <div class="metric-card__value">
                <span class="metric-card__number">{{ detail.tasks.total }}</span>
                <span class="metric-card__sub">{{ $t('dashboard.metric.total') }}</span>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    async load() {
      ...
      this.error = null
      try {
        const res = await getProjectDetail(this.id, { windowDays: this.windowDays })
        if (res.success) {
