...
class WebSocketService {
  ...
  /**
   * Subscribe to task phase transition events
   * @param {number} taskId - The task ID
   * @param {function} callback - Callback function for transition messages
   * @returns {object} Subscription object
   */
  subscribeToPhaseTransition(taskId, callback) {
    if (!this.client || !this.connected) {
      console.warn('WebSocket not connected')
      return null
    }

    const topic = `/topic/task/${taskId}/transition`
    ...
    const subscription = this.client.subscribe(topic, (message) => {
      try {
        const data = JSON.parse(message.body)
        console.log('[WebSocket] Phase transition event:', data)
        callback(data)
      } catch (e) {
        console.error('Failed to parse phase transition message:', e)
      }
    })
  ...
  /**
   * Unsubscribe from task phase transition events
   * @param {number} taskId - The task ID
   */
  unsubscribeFromPhaseTransition(taskId) {
    const topic = `/topic/task/${taskId}/transition`
    if (this.subscriptions.has(topic)) {
      this.subscriptions.get(topic).unsubscribe()
      this.subscriptions.delete(topic)
  ...
  /**
   * Subscribe to all task phase transitions for a project
   * @param {function} callback - Callback function for transition messages
   * @returns {object} Subscription object
   */
  subscribeToAllPhaseTransitions(callback) {
    if (!this.client || !this.connected) {
      console.warn('WebSocket not connected')
      return null
    }

    const topic = `/topic/phase-transitions`
