...
function createTempStorageRoot() {
  return fs.mkdtemp(path.join(os.tmpdir(), 'kanban-iteration-service-'));
}
...
test.test('IterationService.delete removes the iteration and clears linked task iteration ids', async () => {
  ...
  try {
    const { IterationService } = await import(`../src/services/iterationService.js?case=delete-${Date.now()}`);
...
test.test('IterationService.delete returns false when the iteration does not exist', async () => {
  ...
  try {
    const { IterationService } = await import(`../src/services/iterationService.js?case=missing-${Date.now()}`);
    const service = new IterationService();
