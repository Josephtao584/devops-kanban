...
const agents = {
  ...
  security: {
    ...
    description: '负责安全漏洞分析、CVE评估、安全补丁设计与安全验证',
    enabled: true,
    skillNames: ['superpowers', 'git'],
    mcpServerNames: ['context7', 'github'],
  },
...
const templates = {
  'workflow-v1': {
    ...
    name: '默认工作流',
    steps: [
      { id: 'solution-design', name: '头脑风暴设计', instructionPrompt: '使用头脑风暴Skill思考。遇到不熟悉的库或框架时，使用 context7 MCP 查询最新文档。先完成 Spec-Driven 方案设计。必须基于任务上下文输出可直接执行的设计结果，并将设计文档写入 docs：1）明确目标、范围与验收标准；2）梳理用户场景、边界条件与异常场景；3）说明关键约束、依赖与风险；4）给出实现思路、模块改动点与任务拆分；5）定义测试策略、关键测试用例与完成标准；6）列出需要前置确认的问题。最后输出可供下游步骤直接消费的 summary。', agentName: '架构师' },
  ...
  'dev-feature-sdd-tdd-v1': {
    ...
    steps: [
      ...
      { id: 'feature-real-frontend-validation', name: '真实前端验收', instructionPrompt: '你当前负责标准功能开发交付的真实前端验收阶段。使用 Playwright MCP 在真实浏览器中执行页面交互验证。目标：在真实页面和真实交互上下文中确认功能可用且满足验收标准。执行要求：1）说明验证环境、页面入口和操作路径；2）使用 Playwright 验证核心路径、边界交互和异常交互；3）记录页面表现、交互反馈、发现的问题和未覆盖范围；4）明确当前结果是否达到进入交付评审的条件。禁止事项：1）不要仅凭单元测试、集成测试或 mock 结果给通过结论；2）不要忽略 UI/交互问题；3）不要模糊说明验证范围。输出要求：最终只输出 summary，并严格包含以下小节：本步结论（通过/有风险通过/不通过/需补充信息）、已完成内容、交付物清单、未完成或范围外内容、风险与影响、阻塞与待确认问题、给下游步骤的交接建议。', agentName: '测试工程师' },
  ...
  'bugfix-sdd-tdd-v1': {
    ...
    steps: [
      ...
      { id: 'bugfix-real-frontend-validation', name: '真实前端复现与验收', instructionPrompt: '你当前负责缺陷修复与防回归交付的真实前端复现与验收阶段。使用 Playwright MCP 在真实浏览器中执行页面操作和验证。目标：在真实界面中确认原问题无法复现，并检查相邻交互链路。执行要求：1）说明验证环境、页面入口、原始复现路径和新的验证结果；2）使用 Playwright 按原始复现路径操作，确认原问题在真实界面中已不可复现；3）检查相邻功能、边界交互和异常交互是否受影响；4）记录发现的问题、限制和未覆盖范围。禁止事项：1）不要仅凭自动化测试结果给出通过结论；2）不要忽略相邻交互链路；3）不要模糊说明复现和验证过程。输出要求：最终只输出 summary，并严格包含以下小节：本步结论（通过/有风险通过/不通过/需补充信息）、已完成内容、交付物清单、未完成或范围外内容、风险与影响、阻塞与待确认问题、给下游步骤的交接建议。', agentName: '测试工程师' },
  ...
  'vuln-fix-sdd-tdd-v1': {
    ...
    name: '安全漏洞修复',
    steps: [
      { id: 'vuln-cve-analysis', name: 'CVE 漏洞分析与影响评估', instructionPrompt: '你当前负责安全漏洞修复的 CVE 漏洞分析与影响评估阶段。使用 WebSearch 工具查询 CVE 详细信息。目标：明确漏洞详情并从两个维度评估影响。执行要求：1）从任务标题和描述中提取 CVE 编号，使用 WebSearch 查询 NVD、CVE Details、GitHub Advisory Database、厂商安全公告等来源，收集 CVE 编号、漏洞描述、CVSS 评分 v3.x 和严重等级、漏洞类型、受影响的组件/库及其版本范围、修复版本、攻击向量；2）维度一-漏洞对项目的影响：检查项目依赖文件确认是否使用受影响依赖，搜索项目中调用受影响 API 的代码，评估实际调用路径，给出项目级风险评级（高/中/低/无影响）；3）维度二-升级对项目的影响：评估版本跨度（补丁/次/主版本），使用 WebSearch 查询 CHANGELOG 识别 Breaking Changes，评估升级影响程度。禁止事项：1）不要跳过攻击向量分析；2）不要低估影响范围；3）不要忽略间接影响。输出要求：使用 AskUserQuestion 工具向用户展示完整评估结果，选项包含「确认，继续制定方案」和「跳过，该漏洞不影响本项目」，在问题中附上双维度评估摘要。最终只输出 summary，包含：本步结论、已完成内容、交付物清单、未完成或范围外内容、风险与影响、阻塞与待确认问题、给下游步骤的交接建议。', agentName: '安全工程师', requiresConfirmation: false },
      { id: 'vuln-patch-design', name: '升级/补丁方案设计', instructionPrompt: '你当前负责安全漏洞修复的升级/补丁方案设计阶段。使用 WebSearch 工具查询版本变更信息。目标：基于漏洞分析结果制定安全可行的修复方案。执行要求：1）确认当前项目使用的受影响依赖版本，查找修复 CVE 的最低安全版本和最新稳定版本；2）使用 WebSearch 查询目标版本的 CHANGELOG、Release Notes、Migration Guide，分析 Breaking Changes 对当前项目代码的影响；3）制定修复方案：推荐升级目标版本（优先最低安全版本），列出需要修改的代码文件和适配点，评估升级风险（平滑/需适配/需架构变更）；4）设计回滚方案。禁止事项：1）不要设计过度工程化方案；2）不要忽略已有安全中间件或框架能力；3）不要遗漏配置层面修复。输出要求：使用 AskUserQuestion 工具向用户展示修复方案，选项包含「确认方案并执行」和「需要调整方案」，在问题中附上完整方案详情。最终只输出 summary，包含：本步结论、已完成内容、交付物清单、未完成或范围外内容、风险与影响、阻塞与待确认问题、给下游步骤的交接建议。', agentName: '安全工程师', requiresConfirmation: false },
      ...
      { id: 'vuln-implementation', name: '开发实现', instructionPrompt: '你当前负责安全漏洞修复的开发实现阶段。目标：根据确认的升级/补丁方案执行代码修改和依赖升级，确保单测通过。执行要求：1）更新依赖文件中的版本号，执行安装命令（如适用）；2）根据方案中识别的 Breaking Changes 逐个修改受影响代码，确保符合新版本 API 规范；3）优先使用框架/库的安全能力（如参数化查询、模板引擎自动转义）；4）执行项目单测，确保所有测试通过；5）如果单测失败，分析原因并修复。禁止事项：1）不要修改与本次修复无关的代码；2）不要引入新的安全风险；3）不要做无关重构。输出要求：最终只输出 summary，包含：本步结论、已完成内容、交付物清单、未完成或范围外内容、风险与影响、阻塞与待确认问题、给下游步骤的交接建议。', agentName: '后端开发' },
      ...
      { id: 'vuln-delivery-deploy', name: '修复交付与 CI/CD 部署', instructionPrompt: '你当前负责安全漏洞修复的修复交付与 CI/CD 部署阶段。使用 GitHub MCP 查看代码变更、创建发布 PR 或管理部署分支。目标：综合安全分析、测试验证结果，判断是否具备发布条件，并制定部署执行方案。执行要求：1）检查漏洞修复证据链完整性：漏洞分析→方案设计→安全测试→开发实现→回归验证，每一步结论是否明确且一致；2）使用 GitHub MCP 查看最终代码 diff，确认修改范围与方案一致，无无关改动；3）评估发布风险：补丁是否需要灰度发布、是否可能影响性能、是否有回滚方案；4）制定部署策略：立即发布还是定时发布、是否需要通知用户、是否需要先同步到预发布环境；5）说明监控和告警需求：上线后需要关注哪些指标、发现异常时的回滚操作步骤。禁止事项：1）不要隐藏残余风险；2）不要缺少具体部署操作建议；3）不要忽略监控和告警需求。输出要求：最终只输出 summary，包含：本步结论（通过/有风险通过/不通过/需补充信息）、已完成内容、交付物清单（必须包含部署方案文档）、未完成或范围外内容、风险与影响、阻塞与待确认问题、给下游步骤的交接建议。', agentName: 'CI/CD 部署工程师' },
...
const presets = [
  ...
  },
  {
    ...
    agentKeys: ['security', 'architect', 'backend', 'qa', 'deploy'],
    skillKeys: ['git', 'superpowers', 'tdd', 'playwright'],
    mcpKeys: ['context7', 'github', 'playwright'],
  },
];
