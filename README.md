...
| 新瓶颈 | 研发阶段 | Harness 应对 |
| --- | --- | --- |
| 需求进来了没人管 | 需求阶段 | 外部源同步 + Epic 拆解 + 需求澄清 Workflow |
| 需求拆不成可执行的任务 | 任务拆分 | 模板预置任务拆解规则 + AI 辅助分解 |
| 任务来了等人工分配和启动 | 调度阶段 | 自动调度引擎：定时同步 + 自动匹配 Workflow + 自动启动 + 自动通知 |
| 多个任务同时跑互相冲突 | 开发阶段 | Worktree 隔离 + 冲突检测 + 写队列串行化 |
| 做完没人验证质量 | 测试阶段 | 自动测试 + 质量扫描 + 用例生成 |
| 产出太多审查看不过来 | 审查阶段 | AI 审查 + 安全扫描 + 规范检查 |
| 上线后问题发现到修复太慢 | 运维阶段 | 自愈闭环：监控 → 分诊 → 工单 → 修复 → 验证 |
